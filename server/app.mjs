...
function unusedDefaultAlias(db) {
  for (let attempt = 0; attempt < 20; attempt += 1) {
    const alias = makeDefaultAlias();
...
export function createApp({ db, now = () => new Date(), appOrigin, cookieSecure = false, development = false, limit = true, trustProxy = false }) {
  ...
  app.use('/api', (req, res, next) => {
    ...
    if (req.get('x-journal-request') !== '1' || (origin && origin !== allowed) || req.get('sec-fetch-site') === 'cross-site') {
      return res.status(403).json({ error: '请求来源无效，请从应用页面操作。' });
    }
    if (!req.is('application/json')) return res.status(415).json({ error: '请使用 JSON 格式。' });
    next();
  });
  app.use(express.json({ limit: '8kb' }));
  if (limit) app.use('/api', rateLimit({ windowMs: 60_000, limit: 240, standardHeaders: 'draft-8', legacyHeaders: false, message: { error: '操作太频繁，请稍后再试。' } }));
  ...
  const auth = (req, res, next) => {
    ...
    if (!user) return res.status(401).json({ error: '匿名身份已失效，请刷新页面重新进入。' });
    req.user = user;
    next();
  };
  app.get('/api/health', (_req, res) => res.json({ ok: true }));
  const sessionLimit = limit ? rateLimit({ windowMs: 60 * 60_000, limit: 30, skip: req => Boolean(readUser(req)), message: { error: '创建身份过于频繁，请稍后再试。' }, standardHeaders: 'draft-8', legacyHeaders: false }) : (_req, _res, next) => next();
  app.post('/api/session', sessionLimit, (req, res) => {
    let user = readUser(req);
    if (!user) {
      const token = randomBytes(32).toString('hex');
      const id = randomUUID();
      const alias = unusedDefaultAlias(db);
  ...
  app.get('/api/records', auth, (req, res) => {
    ...
    if (!validMonth(month)) return res.status(400).json({ error: '月份格式无效。' });
  ...
  app.put('/api/records/:date', auth, (req, res) => {
    if (!validDay(req.params.date, todayInShanghai(now()))) return res.status(400).json({ error: '请选择 2000 年以来的有效日期，不能记录未来。' });
    if (!bodyIsObject(req.body) || !['success', 'declined'].includes(req.body.outcome) || (req.body.note !== undefined && (typeof req.body.note !== 'string' || req.body.note.length > 300))) return res.status(400).json({ error: '请选择当天结果，备注最多 300 字。' });
  ...
  app.delete('/api/records/:date', auth, (req, res) => {
    if (!validDay(req.params.date, todayInShanghai(now()))) return res.status(400).json({ error: '日期无效。' });
    db.prepare('DELETE FROM records WHERE user_id = ? AND date = ?').run(req.user.id, req.params.date);
    res.json({ ok: true });
  });
  app.patch('/api/profile', auth, (req, res) => {
    if (!bodyIsObject(req.body)) return res.status(400).json({ error: '个人资料格式无效。' });
    const keys = Object.keys(req.body);
    if (!keys.length || keys.some(key => !['alias', 'participating'].includes(key))) return res.status(400).json({ error: '个人资料格式无效。' });
    if ('participating' in req.body && typeof req.body.participating !== 'boolean') return res.status(400).json({ error: '请选择是否参与匿名排行。' });

    let nextAlias = req.user.alias;
    if ('alias' in req.body) {
      if (typeof req.body.alias !== 'string') return res.status(400).json({ error: '用户名格式无效。' });
      if (!req.body.alias.trim()) {
        nextAlias = unusedDefaultAlias(db);
      } else {
        const validated = validateAlias(req.body.alias);
      ...
      if (existing && existing.id !== req.user.id) return res.status(409).json({ error: '这个用户名已经有人使用了，请换一个。' });
    }
  ...
  app.get('/api/leaderboard', (req, res) => {
    ...
    if (!['success', 'declined'].includes(metric) || !['month', 'year', 'all'].includes(period)) return res.status(400).json({ error: '排行榜筛选条件无效。' });
    const user = readUser(req);
    const today = todayInShanghai(now());
  ...
  app.delete('/api/account', auth, (req, res) => {
    if (req.body?.confirmation !== '删除') return res.status(400).json({ error: '请输入「删除」确认。' });
  ...
  app.use((err, _req, res, _next) => {
    if (err.type === 'entity.too.large') return res.status(413).json({ error: '提交内容太长。' });
    if (err instanceof SyntaxError && err.status === 400) return res.status(400).json({ error: '提交内容格式无效。' });
